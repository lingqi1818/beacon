...
static char* ngx_http_beacon_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);
static ngx_int_t ngx_http_beacon_init(ngx_conf_t *cf);
static ngx_int_t ngx_http_beacon_header_filter(ngx_http_request_t *r);
static ngx_command_t ngx_http_beacon_commands[] =
{
  {
    ngx_string("beacon"),
    ...
    offsetof(ngx_http_beacon_loc_conf_t, beacon_switch),
    NULL
  },
  ngx_null_command
};

static ngx_http_module_t ngx_http_beacon_module_ctx =
{
  NULL,
  ngx_http_beacon_init,
  NULL,
  NULL,
  NULL,
  NULL,
  ngx_http_beacon_create_loc_conf,
  ngx_http_beacon_merge_loc_conf
};

ngx_module_t ngx_http_beacon_module =
{
  NGX_MODULE_V1,
  &ngx_http_beacon_module_ctx,
  ngx_http_beacon_commands,
  NGX_HTTP_MODULE,
  NULL,
  NULL,
  NULL,
...
static ngx_int_t ngx_http_beacon_header_filter(ngx_http_request_t *r)
{
	...
	for (i = 0; i < r->headers_in.cookies.nelts; i++) {
		...
		if (*(p + sizeof("beacon_id") -1 ) == ' ' || *(p + sizeof("beacon_id") -1) == '=') {
			break;
		}
	}
...
not_found:
	...
	if (set_cookie == NULL) {
		ngx_log_error(NGX_LOG_EMERG, r->connection->log, 0, "ngx_list_push set_cookie failed !!!");
	}
	set_cookie->hash = 1;
	...
	set_cookie->value.len = sizeof("beacon_id=") - 1
							  ...
							  +	sizeof(";expires=")-1
							  +	sizeof("Thu, 31 Dec 2115 23:59:59 GMT")-1
							  +	sizeof(";max-age=")-1
							  +	sizeof("3153600000")-1
							  + sizeof(";domain=") - 1
							  + domain_len
	...
	pp = ngx_cpymem(pp, "Thu, 31 Dec 2115 23:59:59 GMT", sizeof("Thu, 31 Dec 2115 23:59:59 GMT") - 1);
...
void gen_beacon_id(ngx_str_t *beacon_id,ngx_http_request_t *r){
	...
	ngx_sprintf(beacon_id->data,"%s-%XL-%XL-%XL",loc_addr.data,ngx_pid,us,rnd);
